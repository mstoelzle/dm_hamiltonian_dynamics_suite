...
class TestToyDataset(parameterized.TestCase):
    ...
    def compare_structures_all_the_same(self, example, batched_example):
        ...
        example["image"] = tf.image.convert_image_dtype(
            example["image"], dtype=batched_example["image"].dtype
        ).numpy()
        for v1, v2 in zip(jax.tree_leaves(example), jax.tree_leaves(batched_example)):
    ...
    def test_dataset(
        ...
        examples = tuple(x for x in ds())
        self.assertEqual(
            len(test_examples), len(examples), "Number of test examples not the same."
        )
        # Compare individual examples
