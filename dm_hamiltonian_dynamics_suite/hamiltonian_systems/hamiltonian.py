...
Integrator = Callable[
    [
        Union[
            phase_space.HamiltonianFunction, phase_space.SymplecticTangentFunction
        ],  # dy_dt
...
class HamiltonianSystem(abc.ABC):
    ...
    def hamiltonian_from_params(
        self, params: utils.Params, **kwargs: Any
    ...
    def random_offset_bounds(self) -> utils.BoxRegion:
        ...
        return utils.BoxRegion(minimum=-extra_size, maximum=extra_size)
    ...
    def simulate_scipy(
        ...
        **kwargs: Any,
        ...
        solution = integrate.solve_ivp(
            fun=fun, t_span=t_span, y0=y0, t_eval=t_eval, **kwargs
        )
    ...
    def generate_trajectories_dt(
        ...
        **kwargs: Any,
        ...
        if num_steps_forward < 0 or num_steps_backward < 0:
            raise ValueError(
                "num_steps_forward and num_steps_backward can not be " "negative."
            )
        if num_steps_forward == 0 and num_steps_backward == 0:
            raise ValueError(
                "You need one of num_steps_forward or "
                "num_of_steps_backward to be positive."
            )
        if num_steps_forward > 0 and num_steps_backward > 0 and not include_t0:
            raise ValueError(
                "When both num_steps_forward and num_steps_backward are "
    ...
    def generate_and_render_dt(
        ...
        **kwargs: Any,
        ...
        if within_canvas_bounds and (y0 is not None or params is not None):
            raise ValueError(
                "Within canvas bounds is valid only when y0 and params " "are None."
            )
        if params is None:
        ...
        if within_canvas_bounds:
            ...
            while len(valid) < num_trajectories:
                for idx in range(x.q.shape[0]):
                    ...
                    if jnp.all(position >= self.canvas_bounds().min) and jnp.all(
                        position <= self.canvas_bounds().max
                    ):
                ...
                x = self.generate_trajectories_dt(
                    y0, t0, dt, params, num_steps, **kwargs
                )
                x = jax.tree.map(lambda x_: jnp.swapaxes(x_, 0, 1), x)
                x = jax.tree.map(
                    lambda i, j:  # pylint:disable=g-long-lambda
                    jnp.concatenate([i[:, None], j], axis=1),
                    y0,
                    x,
                )
            x, params = jax.tree.map(lambda *args: jnp.stack(args, axis=0), *valid)

        hamiltonian = self.hamiltonian_from_params(params, **kwargs)
        df_dt = jax.vmap(
            phase_space.poisson_bracket_with_q_and_p(hamiltonian),
            in_axes=[0, 1],
            out_axes=1,
        )
...
class TimeIndependentHamiltonianSystem(HamiltonianSystem):
    ...
    @abc.abstractmethod
    def _hamiltonian(
        self, y: phase_space.PhaseSpace, params: utils.Params, **kwargs: Any
    ) -> jnp.ndarray:
