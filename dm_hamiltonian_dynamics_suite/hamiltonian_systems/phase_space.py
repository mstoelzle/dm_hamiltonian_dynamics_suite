...
class TangentPhaseSpace(PhaseSpace):
    ...
    def __add__(
        ...
        if isinstance(other, TangentPhaseSpace):
            return TangentPhaseSpace(
                position=self.q + other.q, momentum=self.p + other.p
            )
        elif isinstance(other, PhaseSpace):
            return PhaseSpace(position=self.q + other.q, momentum=self.p + other.p)
        else:
            raise ValueError(f"Can not add TangentPhaseSpace and {type(other)}.")

    def __radd__(
        self, other: Union[PhaseSpace, "TangentPhaseSpace"]
    ) -> Union[PhaseSpace, "TangentPhaseSpace"]:
        return self.__add__(other)

    def __mul__(self, other: jnp.ndarray) -> "TangentPhaseSpace":
        return TangentPhaseSpace(position=self.q * other, momentum=self.p * other)
