...
class BoxRegion:
    ...
    def __init__(self, minimum: FloatArray, maximum: FloatArray):
        ...
        if minimum.shape != maximum.shape:
            raise ValueError(
                f"The passed values for minimum and maximum should have "
...
def dt_to_t_eval(t0: FloatArray, dt: FloatArray, num_steps: int) -> jnp.ndarray:
    if isinstance(t0, (float, np.ndarray)) and isinstance(dt, (float, np.ndarray)):
...
def simple_loop(
    ...
    if t_args is None and num_steps is None:
        raise ValueError(
            "Exactly one of `t_args` and `num_steps` should be " "provided."
        )
    ...
    for i in range(num_steps):
        x, _ = step(x, t_args[i])
        y.append(x)
    return jax.tree.map(lambda *args: jnp.stack(args, axis=0), *y)
...
def hsv2rgb(array: jnp.ndarray) -> jnp.ndarray:
    ...
    hi_is_5 = hi == 5
    out = (
        hi_is_0 * jnp.stack((v, t, p), axis=-1)
...
multi_shuffle = jax.vmap(
    lambda x, key, k: jnr.permutation(key, x)[:k], in_axes=(0, 0, None), out_axes=0
)


def random_int_k_from_n(
    rng: jnp.ndarray, num_samples: int, n: int, k: int
) -> jnp.ndarray:
    """Samples randomly k integers from 1 to n."""
    if k > n:
        raise ValueError(
            f"k should be less than or equal to n, but got k={k} and " f"n={n}."
        )
