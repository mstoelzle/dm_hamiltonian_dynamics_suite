...
class PotentialFreeSystem(hamiltonian.TimeIndependentHamiltonianSystem):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if eigen_values_range.dims != 0 and eigen_values_range.dims != system_dims:
            raise ValueError(
                f"The eigen_values_range must be of the same dimensions "
        ...
        if init_vector_range.dims != 0 and init_vector_range.dims != system_dims:
            raise ValueError(
                f"The init_vector_range must be of the same dimensions "
                f"as the system dimensions, but is "
    ...
    def sample_y(
        ...
        **kwargs: Any,
        ...
        # Sample random state
        y = jnr.uniform(rng_key, [num_samples, 2 * self.system_dims], dtype=self.dtype)
