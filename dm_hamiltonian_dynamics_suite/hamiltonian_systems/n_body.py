...
class NBodySystem(hamiltonian.TimeIndependentHamiltonianSystem):
    ...
    def sample_params(
        ...
        m = jax.random.uniform(
            key1,
            [num_samples, self.n],
            minval=self.m_range.min,
            maxval=self.m_range.max,
        )
        g = jax.random.uniform(
            key2, [num_samples], minval=self.g_range.min, maxval=self.g_range.max
        )
        return dict(m=m, g=g)
        ...
        **kwargs: Any,
        ...
        if self.num_colors == 1:
            ...
            if self.num_colors < self.n:
                raise ValueError(
                    "The number of colors must be at least the number of "
                    "objects or 1."
                )
            color_index = utils.random_int_k_from_n(
                key2, num_samples=n, n=self.num_colors, k=self.n
            )
