...
class IdealPendulum(hamiltonian.TimeIndependentHamiltonianSystem):
    ...
    def sample_params(
        ...
        m = jax.random.uniform(
            key1, [num_samples], minval=self.m_range.min, maxval=self.m_range.max
        )
        l = jax.random.uniform(
            key2, [num_samples], minval=self.l_range.min, maxval=self.l_range.max
        )
        g = jax.random.uniform(
            key3, [num_samples], minval=self.g_range.min, maxval=self.g_range.max
        )
