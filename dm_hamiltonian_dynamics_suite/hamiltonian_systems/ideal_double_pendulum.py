...
class IdealDoublePendulum(hamiltonian.TimeIndependentHamiltonianSystem):
    ...
    def sample_y(
        ...
        **kwargs: Any,
        ...
        state = jnp.stack(
            [state_1[..., 0], state_2[..., 0], state_1[..., 1], state_2[..., 1]],
            axis=-1,
        )
    ...
    def sample_params(
        ...
        m_1 = jax.random.uniform(
            keys[0], [num_samples], minval=self.m_range.min, maxval=self.m_range.max
        )
        m_2 = jax.random.uniform(
            keys[1], [num_samples], minval=self.m_range.min, maxval=self.m_range.max
        )
        l_1 = jax.random.uniform(
            keys[2], [num_samples], minval=self.l_range.min, maxval=self.l_range.max
        )
        l_2 = jax.random.uniform(
            keys[3], [num_samples], minval=self.l_range.min, maxval=self.l_range.max
        )
        g = jax.random.uniform(
            keys[4], [num_samples], minval=self.g_range.min, maxval=self.g_range.max
        )
        ...
        **kwargs: Any,
        ...
        if self.num_colors == 1:
            ...
            color_index = utils.random_int_k_from_n(
                key2, num_samples=n, n=self.num_colors, k=2
            )
