...
def sample_from_simplex(
    num_samples: int, rng: jnp.ndarray, dim: int = 3, vmin: float = 0.0
) -> jnp.ndarray:
...
class ZeroSumGame:
    ...
    def __init__(
        self, game_name: str, dynamics: str = "replicator", method: str = "scipy"
    ):
    ...
    def generate_trajectories(
        self, x0: jnp.ndarray, t0: utils.FloatArray, t_eval: jnp.ndarray
    ) -> jnp.ndarray:
    ...
    def render_trajectories(self, x: jnp.ndarray) -> jnp.ndarray:
        ...
        x_1 = x[..., :nrows]
        x_2 = x[..., nrows:]
        x_1 = x_1.repeat(ncols, axis=-1).reshape(
            x.shape[:-1]
            + (
                nrows,
                ncols,
        ...
        x_2 = x_2.repeat(nrows, axis=-1).reshape(
            x.shape[:-1]
            + (
                nrows,
                ncols,
        ...
        temp_image = [
            tile_array(
                x,
                np.ceil(32 / x.shape[0]).astype("int"),
                np.ceil(32 / x.shape[1]).astype("int"),
            )[:32, :32]
