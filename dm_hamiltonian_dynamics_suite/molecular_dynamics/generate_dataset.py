...
def render_images(
    x: Array, box_length: float, resolution: int = 32, particle_radius: float = 0.3
) -> Array:
    ...
    if num_coordinates % (2 * dim) != 0:
        raise ValueError(
            "Expected the number of coordinates to be divisible by 4, "
    ...
    box_region = utils.BoxRegion(-box_length / 2.0, box_length / 2.0)
...
def convert_sequence(sequence: Array, box_length: float) -> Mapping[str, Array]:
    ...
    if num_fields != 6:
        raise ValueError(
            "Expected input sequence to be of shape "
