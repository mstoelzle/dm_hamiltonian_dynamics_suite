...
def save_features(
    file: TextIO, example_dict: Mapping[str, Any], prefix: str = ""
) -> None:
        ...
        if isinstance(v, dict):
            ...
            if isinstance(v, (onp.ndarray, jnp.ndarray)):
                ...
                raise NotImplementedError(
                    f"Currently the only supported feature types "
...
def transform_dataset(
    ...
    if os.path.exists(file_path):
        if not overwrite:
            ...
            logging.info(
                "The file with prefix %s already exist and overwrite=True."
                " Deleting.",
                prefix,
            )
            os.remove(file_path)
    with tf.io.TFRecordWriter(file_path) as writer:
        for element in generator:
            ...
            example = tf.train.Example(
                features=tf.train.Features(feature=encode_example(element))
            )
...
def no_open_spiel_func(*_, **__):
    raise ValueError(
        "You must download and install `open_spiel` first in "
...
if not open_spiel_available():
    ...
    MATCHING_PENNIES = (game_dynamics.ZeroSumGame, lambda: dict(game_name="matrix_mp"))
    ROCK_PAPER_SCISSORS = (
        game_dynamics.ZeroSumGame,
